...
function menu() {
	...
	$layout .= "<div id='logo_secundario' class='item'>
            <a href='{$dados['url']['inicio']}'><img src='{$dados['url']['inicio']}/img/logo_bsrun_branco.png' alt='BSRUN Assessoria Esportiva' title='BSRUN Assessoria Esportiva'/></a>
        </div>";
	$layout .= "<div class='item'><ul class='item left flex'><li class=''><span data-icon='menu' class='bt-menu left'>MENU</span></li>";	
		for($a=0;$a<count($dados_menu);$a++){
			if($dados_menu[$a]['sec'] != 'home'){
				$layout .= "<li class='w{$n_items}'><a href='{$dados['url']['inicio']}/{$dados_menu[$a]['sec']}'>{$dados_menu[$a]['sec_titulo']}</a></li>";    
			}
		}
	$layout .= "</ul></div></nav>";	
	}
